...
- создаем цикл for, который перебирает список всех файлов .txt на .log.
Вторая строка применяется к каждому элементу списка и перемещает файл в новый, заменяя его .txt на .log. Деталь ${file%.txt}
